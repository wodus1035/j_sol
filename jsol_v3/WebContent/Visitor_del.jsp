...
//int[] selectedcell = Arrays.stream(request.getParameterValues("selected")).mapToInt(Integer::parseInt).toArray();
int[] selectedcell = new int[request.getParameterValues("selected").length];
for(int i = 0; i < request.getParameterValues("selected").length; i ++){
	selectedcell[i] = Integer.parseInt(request.getParameterValues("selected")[i]);
}
if(request.getParameter("selected") != null){

	BoardDAO dao = BoardDAO.getInstance();
	int check = dao.Visitor_del(selectedcell);
	
	if(check>=1){
		for(int i = 0; i < request.getParameterValues("selected").length; i ++){
			application.log("방문자 삭제 : " + selectedcell[i]);
		}
