...
Member admin_list = dao.getadmin_Member(c_number, phone);
String name = admin_list.getName();
String purpose = admin_list.getPurpose();
String is_sign = admin_list.getIs_sign();
String birth = admin_list.getBirth();
long m_number = admin_list.getM_number();
...
<div class="card shadow-lg border-0 rounded-lg">
								...
								<div class="card-body">
									<form id="admin_modify" name="admin_modify" method="post" action="admin_modify_pro.jsp" onsubmit="return checkValue()">
										<input type="hidden" id="procMode" name="procMode" value=""/>
										<input type="hidden" id="m_number" name="m_number" value="<%=m_number%>"/>
										
										<div class="form-group"><label class="small mb-1" for="c_name">이름</label>
											<input class="form-control py-4" id="name" name="name" type="text" value="<%=name%>"/>
										</div>
										<div class="form-group"><label class="small mb-1" for="c_birth">생년월일</label>		
											<input class="form-control py-4" id="birth" name="birth" type="text" 
											placeholder="ex)000000" maxlength="6" oninput=" numberMaxLength(this);" numberOnly value="<%=birth%>"/>
										</div>
										...
										<div class="form-group"><label class="small mb-1" for="c_type">개인정보동의</label>
											<select name="is_sign" id="is_sign" class="g2-form-control col-sm-2 fs12">
												<option value="Y" <%if("Y".equals(admin_list.getIs_sign())){ %>selected<%} %>>동의</option>
												<option value="N" <%if("N".equals(admin_list.getIs_sign())){%>selected<%} %>>비동의</option>
											</select>
										</div>
										</div>
										<div class="row">
